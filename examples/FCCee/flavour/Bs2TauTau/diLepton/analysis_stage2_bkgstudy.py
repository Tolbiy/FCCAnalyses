...
processList_test = {
    'p8_ee_Zbb_ecm91':{'chunks':1, 'fraction':2e-4},
...
outputDir   = "../../../../../../../../../../../work/t/tomonnar/public/FCCeePhysicsPerformance/case-studies/flavour/Bs2TauTau/LeptonicModes/merged/Stage2/Background_Study/eede"
...
class RDFanalysis():
    ...
    def analysers(df):
        df2 = (
            df
               ...
               #Stage 1 cuts
               #.Filter("EVT_ThrustEmin_E < 38 && recoEmiss_e > 10 && EVT_ThrustEmin_Eneutral < 10 && n_lepton > 1 && has_dilepton > 0 && has_dilepton_SameSide > 0 && has_dilepton_SigHemi > 0 && has_dilepton_OppositeCharges > 0 && has_dilepton_Vertex == 0")
               ...
               .Define("MC_dilepton_ind","FCCAnalyses::ZHfunctions::Finding_MC_dimuon(dilepton_ind,Lepton0,MCRecoAssociations0,MCRecoAssociations1)") #This function might not work as intended if the dimuon is made of muon not coming from Bs2TauTau (can happen by chance if 3 muon in sig hemi)
               .Define("n_MC_dilepton","MC_dilepton_ind.size()") #Check to see if the truth-matching in the RECO->MC direction can fail (== 2 -> ok else failed)

               .Define("MC_dilepton_CA_ind","FCCAnalyses::ZHfunctions::Find_MC_CommonAncestor(MC_dilepton_ind,Particle,Particle0)")
               .Define("MC_dilepton_CADaughters_ind","FCCAnalyses::ZHfunctions::Find_MC_CommonAncestor_Daughters(MC_dilepton_CA_ind,Particle,Particle1)") #Always check the indices value before looking into the collection (-1 for failed searches)
               
               
               #.Define("MC_dilep1_NoCADaughters_ind","FCCAnalyses::ZHfunctions::Find_MC_NoCommonAncestor_Daughters(MC_dilepton_CA_ind,MC_dilepton_ind,Particle,Particle0,0)")
               #.Define("MC_dilep2_NoCADaughters_ind","FCCAnalyses::ZHfunctions::Find_MC_NoCommonAncestor_Daughters(MC_dilepton_CA_ind,MC_dilepton_ind,Particle,Particle0,1)")

               .Define("Stage1_Pass","if (EVT_ThrustEmin_E < 38 && recoEmiss_e > 10 && EVT_ThrustEmin_Eneutral < 10 && n_lepton > 1 && has_dilepton > 0 && has_dilepton_SameSide > 0 && has_dilepton_SigHemi > 0 && has_dilepton_OppositeCharges > 0 && has_dilepton_Vertex == 0 && EVT_hasPV == 1) return 1; else return 0;")
               .Define("MC_dilepton_CADaughters_ind_reduced","ROOT::VecOps::RVec<int> result; if (Stage1_Pass) result = FCCAnalyses::ZHfunctions::Decay_Chain(MC_dilepton_CADaughters_ind); else result = {-10}; return result;")
               .Define("MC_dileptonplus_Family","ROOT::VecOps::RVec<int> result; if (Stage1_Pass) result = FCCAnalyses::ZHfunctions::Find_MuSubdecays(MC_dilepton_CADaughters_ind_reduced,MC_PDG,+1); else result = {-10}; return result;")
               .Define("MC_dileptonminus_Family","ROOT::VecOps::RVec<int> result; if (Stage1_Pass) result = FCCAnalyses::ZHfunctions::Find_MuSubdecays(MC_dilepton_CADaughters_ind_reduced,MC_PDG,-1); else result = {-10}; return result;")
               .Define("MC_dilepton_BkgCat","if (Stage1_Pass) return 10*FCCAnalyses::ZHfunctions::Find_Categories(MC_dileptonplus_Family) + FCCAnalyses::ZHfunctions::Find_Categories(MC_dileptonminus_Family); else return -99;")
               .Define("Selected","if(EVT_ThrustEmin_E < 38 && recoEmiss_e > 10 && EVT_ThrustEmin_Eneutral < 10 && n_lepton > 1 && has_dilepton > 0 && has_dilepton_SameSide > 0 && has_dilepton_SigHemi > 0 && has_dilepton_OppositeCharges > 0 && has_dilepton_Vertex == 0 && EVT_hasPV == 1 && MC_dilepton_BkgCat == 0) return 1; else return 0;")
    ...
    def output():
        branchList = [
                ...
                "MC_dilepton_ind","n_MC_dilepton","MC_dilepton_CA_ind","MC_dilepton_CADaughters_ind","MC_dilepton_BkgCat",

                #"MC_dilep1_NoCADaughters_ind","MC_dilep2_NoCADaughters_ind"
