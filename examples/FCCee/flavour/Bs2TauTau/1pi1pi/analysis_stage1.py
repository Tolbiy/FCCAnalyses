...
outputDirEos   = "/eos/experiment/fcc/ee/analyses_storage/flavor/Bs2TauTau/flatNtuples/winter2023/analysis_stage1_1pi1pi_withCuts"
...
class RDFanalysis():
    ...
    def analysers(df):
        df2 = (
            df
               ...
               .Filter("recoEmiss_e > 10.0 && EVT_ThrustEmin_E < 38.")
               ...
               #Use perfect PID from PIDatVertex, the threshold for similarity is quite low meaning that some particles that have a mass close to pi will also be taken (not a bad idea)
               ...
               ##################################
               ## Select the 1pi1pi candidates ##
               ##################################

               .Define("Pair_1pi1pi_ind","ROOT::VecOps::RVec<ROOT::VecOps::RVec<int>> res; for (size_t i=0; i<pi_px.size(); ++i){for(size_t j=i+1; j<pi_px.size(); ++j){ROOT::VecOps::RVec<int> temp = {int(i),int(j)}; res.push_back(temp);}} return res;")
               .Define("Pair_1pi1pi_isSigSide",  "ROOT::VecOps::RVec<int> res; for (size_t i=0; i<Pair_1pi1pi_ind.size(); ++i){if (pi_thrustangles.at(Pair_1pi1pi_ind.at(i).at(1)) > 0   &&   pi_thrustangles.at(Pair_1pi1pi_ind.at(i).at(0)) > 0)   res.push_back(1); else res.push_back(0);} return res;")
               .Define("Pair_1pi1pi_isOppositeCharge","ROOT::VecOps::RVec<int> res; for (size_t i=0; i<Pair_1pi1pi_ind.size(); ++i){ if (pi_charge.at(Pair_1pi1pi_ind.at(i).at(0))*pi_charge.at(Pair_1pi1pi_ind.at(i).at(1)) < 0)                    res.push_back(1); else res.push_back(0);} return res;")
               .Define("Pair_1pi1pi_isGood1pi",  "ROOT::VecOps::RVec<int> res; for (size_t i=0; i<Pair_1pi1pi_ind.size(); ++i){if ((pi_energy.at(Pair_1pi1pi_ind.at(i).at(0)) > 2.0 && pi_d0.at(Pair_1pi1pi_ind.at(i).at(0)) > 0.02) && (pi_energy.at(Pair_1pi1pi_ind.at(i).at(1)) > 2.0 && pi_d0.at(Pair_1pi1pi_ind.at(i).at(1)) > 0.02)) res.push_back(1); else res.push_back(0);} return res;")
               .Define("Pair_1pi1pi_isNoVertex", "ROOT::VecOps::RVec<int> res; for (size_t i=0; i<Pair_1pi1pi_ind.size(); ++i){if ((pi_hasVertex.at(Pair_1pi1pi_ind.at(i).at(0)) == 0) && (pi_hasVertex.at(Pair_1pi1pi_ind.at(i).at(1)) == 0))       res.push_back(1); else res.push_back(0);} return res;")

               .Define("ConservativeSel_1pi1pi",  "ROOT::VecOps::RVec<ROOT::VecOps::RVec<int>> res; for (size_t i=0; i<Pair_1pi1pi_ind.size(); ++i) {if (Pair_1pi1pi_isSigSide.at(i) > 0 && Pair_1pi1pi_isOppositeCharge.at(i) > 0 && Pair_1pi1pi_isGood1pi.at(i) > 0) res.push_back(Pair_1pi1pi_ind.at(i));} return res;")
               .Define("ConservativeCand_1pi1pi", "FCCAnalyses::ZHfunctions::SelectCandidates_3pi1pi_Ediff(ConservativeSel_1pi1pi,pi_energy,pi_energy)")
               .Define("OptimisticSel_1pi1pi",   "ROOT::VecOps::RVec<ROOT::VecOps::RVec<int>> res; for (size_t i=0; i<Pair_1pi1pi_ind.size(); ++i) {if (Pair_1pi1pi_isSigSide.at(i) > 0 && Pair_1pi1pi_isOppositeCharge.at(i) > 0 && Pair_1pi1pi_isNoVertex.at(i) > 0) res.push_back(Pair_1pi1pi_ind.at(i));} return res;")
               .Define("OptimisticCand_1pi1pi",  "FCCAnalyses::ZHfunctions::SelectCandidates_3pi1pi_Ediff(OptimisticSel_1pi1pi,pi_energy,pi_energy)")

               .Filter("OptimisticCand_1pi1pi.at(0) > -1 || ConservativeCand_1pi1pi.at(0) > -1")
               ...
               .Define("TM_1piplus_ind","FCCAnalyses::ZHfunctions::TM_Bs2TauTauDaughters_ind(genBs2TauTau_1piplusDaughters_ind,MCRecoAssociations1,MCRecoAssociations0)")
               .Define("nTM_1piplus","if (TM_1piplus_ind.at(0) < 0) return TM_1piplus_ind.at(0); else return int(TM_1piplus_ind.size());")
               .Define("TM_1piplus_px",     "ROOT::VecOps::RVec<float> res; if (TM_1piplus_ind.at(0) < 0) res.push_back(-999.9); else res = ReconstructedParticle::get_px(ReconstructedParticle::get(TM_1piplus_ind,RecoPartPIDAtVertex)); return res;")
               .Define("TM_1piplus_py",     "ROOT::VecOps::RVec<float> res; if (TM_1piplus_ind.at(0) < 0) res.push_back(-999.9); else res = ReconstructedParticle::get_py(ReconstructedParticle::get(TM_1piplus_ind,RecoPartPIDAtVertex)); return res;")
               .Define("TM_1piplus_pz",     "ROOT::VecOps::RVec<float> res; if (TM_1piplus_ind.at(0) < 0) res.push_back(-999.9); else res = ReconstructedParticle::get_pz(ReconstructedParticle::get(TM_1piplus_ind,RecoPartPIDAtVertex)); return res;")
               .Define("TM_1piplus_phi",    "ROOT::VecOps::RVec<float> res; if (TM_1piplus_ind.at(0) < 0) res.push_back(-999.9); else res = ReconstructedParticle::get_phi(ReconstructedParticle::get(TM_1piplus_ind,RecoPartPIDAtVertex)); return res;")
               .Define("TM_1piplus_eta",    "ROOT::VecOps::RVec<float> res; if (TM_1piplus_ind.at(0) < 0) res.push_back(-999.9); else res = ReconstructedParticle::get_eta(ReconstructedParticle::get(TM_1piplus_ind,RecoPartPIDAtVertex)); return res;")
               .Define("TM_1piplus_z0",     "ROOT::VecOps::RVec<float> res; if (TM_1piplus_ind.at(0) < 0) res.push_back(-999.9); else res = ReconstructedParticle2Track::getRP2TRK_Z0(ReconstructedParticle::get(TM_1piplus_ind,RecoPartPIDAtVertex),EFlowTrack_1); return res;")
               .Define("TM_1piplus_d0",     "ROOT::VecOps::RVec<float> res; if (TM_1piplus_ind.at(0) < 0) res.push_back(-999.9); else res = ReconstructedParticle2Track::getRP2TRK_D0(ReconstructedParticle::get(TM_1piplus_ind,RecoPartPIDAtVertex),EFlowTrack_1); return res;")
               .Define("TM_1piplus_energy", "ROOT::VecOps::RVec<float> res; if (TM_1piplus_ind.at(0) < 0) res.push_back(-999.9); else res = ReconstructedParticle::get_e(ReconstructedParticle::get(TM_1piplus_ind,RecoPartPIDAtVertex)); return res;")
               .Define("TM_1piplus_mass",   "ROOT::VecOps::RVec<float> res; if (TM_1piplus_ind.at(0) < 0) res.push_back(-999.9); else res = ReconstructedParticle::get_mass(ReconstructedParticle::get(TM_1piplus_ind,RecoPartPIDAtVertex)); return res;")
               .Define("TM_1piplus_charge", "ROOT::VecOps::RVec<int> res; if (TM_1piplus_ind.at(0) < 0) res.push_back(-999); else res = ReconstructedParticle::get_charge(ReconstructedParticle::get(TM_1piplus_ind,RecoPartPIDAtVertex)); return res;")
               .Define("TM_1piplus_PDG",    "ROOT::VecOps::RVec<int> res; if (TM_1piplus_ind.at(0) < 0) res.push_back(-999); else res = ReconstructedParticle::get_type(ReconstructedParticle::get(TM_1piplus_ind,RecoPartPIDAtVertex)); return res;")
               ...
               .Define("TM_1piminus_ind","FCCAnalyses::ZHfunctions::TM_Bs2TauTauDaughters_ind(genBs2TauTau_1piminusDaughters_ind,MCRecoAssociations1,MCRecoAssociations0)")
               .Define("nTM_1piminus","if (TM_1piminus_ind.at(0) < 0) return TM_1piminus_ind.at(0); else return int(TM_1piminus_ind.size());")
               .Define("TM_1piminus_px",     "ROOT::VecOps::RVec<float> res; if (TM_1piminus_ind.at(0) < 0) res.push_back(-999.9); else res = ReconstructedParticle::get_px(ReconstructedParticle::get(TM_1piminus_ind,RecoPartPIDAtVertex)); return res;")
               .Define("TM_1piminus_py",     "ROOT::VecOps::RVec<float> res; if (TM_1piminus_ind.at(0) < 0) res.push_back(-999.9); else res = ReconstructedParticle::get_py(ReconstructedParticle::get(TM_1piminus_ind,RecoPartPIDAtVertex)); return res;")
               .Define("TM_1piminus_pz",     "ROOT::VecOps::RVec<float> res; if (TM_1piminus_ind.at(0) < 0) res.push_back(-999.9); else res = ReconstructedParticle::get_pz(ReconstructedParticle::get(TM_1piminus_ind,RecoPartPIDAtVertex)); return res;")
               .Define("TM_1piminus_phi",    "ROOT::VecOps::RVec<float> res; if (TM_1piminus_ind.at(0) < 0) res.push_back(-999.9); else res = ReconstructedParticle::get_phi(ReconstructedParticle::get(TM_1piminus_ind,RecoPartPIDAtVertex)); return res;")
               .Define("TM_1piminus_eta",    "ROOT::VecOps::RVec<float> res; if (TM_1piminus_ind.at(0) < 0) res.push_back(-999.9); else res = ReconstructedParticle::get_eta(ReconstructedParticle::get(TM_1piminus_ind,RecoPartPIDAtVertex)); return res;")
               .Define("TM_1piminus_z0",     "ROOT::VecOps::RVec<float> res; if (TM_1piminus_ind.at(0) < 0) res.push_back(-999.9); else res = ReconstructedParticle2Track::getRP2TRK_Z0(ReconstructedParticle::get(TM_1piminus_ind,RecoPartPIDAtVertex),EFlowTrack_1); return res;")
               .Define("TM_1piminus_d0",     "ROOT::VecOps::RVec<float> res; if (TM_1piminus_ind.at(0) < 0) res.push_back(-999.9); else res = ReconstructedParticle2Track::getRP2TRK_D0(ReconstructedParticle::get(TM_1piminus_ind,RecoPartPIDAtVertex),EFlowTrack_1); return res;")
               .Define("TM_1piminus_energy", "ROOT::VecOps::RVec<float> res; if (TM_1piminus_ind.at(0) < 0) res.push_back(-999.9); else res = ReconstructedParticle::get_e(ReconstructedParticle::get(TM_1piminus_ind,RecoPartPIDAtVertex)); return res;")
               .Define("TM_1piminus_mass",   "ROOT::VecOps::RVec<float> res; if (TM_1piminus_ind.at(0) < 0) res.push_back(-999.9); else res = ReconstructedParticle::get_mass(ReconstructedParticle::get(TM_1piminus_ind,RecoPartPIDAtVertex)); return res;")
               .Define("TM_1piminus_charge", "ROOT::VecOps::RVec<int> res; if (TM_1piminus_ind.at(0) < 0) res.push_back(-999); else res = ReconstructedParticle::get_charge(ReconstructedParticle::get(TM_1piminus_ind,RecoPartPIDAtVertex)); return res;")
               .Define("TM_1piminus_PDG",    "ROOT::VecOps::RVec<int> res; if (TM_1piminus_ind.at(0) < 0) res.push_back(-999); else res = ReconstructedParticle::get_type(ReconstructedParticle::get(TM_1piminus_ind,RecoPartPIDAtVertex)); return res;")
    ...
    def output():
        branchList = [
                ...
                "n_pi","pi_px","pi_py","pi_pz","pi_phi","pi_eta","pi_d0","pi_z0","pi_energy","pi_mass","pi_charge","pi_PDG",
                "pi_thrustangles","pi_thrustEmin_n","pi_thrustEmax_n","pi_hasVertex","n_pi_hasNoVertex","pi_charge_noVertex_sighemi","n_pi_charge_noVertex_sighemi",

                "Pair_1pi1pi_ind","Pair_1pi1pi_isSigSide","Pair_1pi1pi_isOppositeCharge","Pair_1pi1pi_isGood1pi","Pair_1pi1pi_isNoVertex",
                "ConservativeSel_1pi1pi","ConservativeCand_1pi1pi","OptimisticSel_1pi1pi","OptimisticCand_1pi1pi",

                "nTM_1piplus","TM_1piplus_px","TM_1piplus_py","TM_1piplus_pz","TM_1piplus_phi","TM_1piplus_eta","TM_1piplus_z0","TM_1piplus_d0","TM_1piplus_energy","TM_1piplus_mass","TM_1piplus_charge","TM_1piplus_PDG",
                "TM_1piplus_thrustangles","TM_1piplus_thrustEmin_n","TM_1piplus_thrustEmax_n","TM_1piplus_hasVertex","n_TM_1piplus_hasVertex",
                "nTM_1piminus","TM_1piminus_px","TM_1piminus_py","TM_1piminus_pz","TM_1piminus_phi","TM_1piminus_eta","TM_1piminus_z0","TM_1piminus_d0","TM_1piminus_energy","TM_1piminus_mass","TM_1piminus_charge","TM_1piminus_PDG",
                "TM_1piminus_thrustangles","TM_1piminus_thrustEmin_n","TM_1piminus_thrustEmax_n","TM_1piminus_hasVertex","n_TM_1piminus_hasVertex",
